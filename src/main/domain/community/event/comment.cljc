...
(s/def ::comment_at int?)

(s/def ::event_id ::domain.community.event/id)
...
(s/def ::query (s/keys :req-un [::id ::event_id ::member ::body ::comment_at]))

(s/def ::member_id ::domain.community.member/id)
(s/def ::command (s/keys :req-un [::event_id ::member_id ::body] :opt-un [::id]))
