...
(defn list-community [{:keys [user-id request-size begin-cursor last-cursor]} repo]
  ...
  (let [sort-order (if (and (nil? begin-cursor) (some? last-cursor)) :created-at-asc :created-at-desc)
        communities (cond
                      ...
                      (domain.community/list-part-community (:community-query-repository repo) request-size nil sort-order)
                      ;; begin-cursor より前 最新 n 件
                      (some? begin-cursor)
                      (domain.community/list-part-community (:community-query-repository repo) request-size begin-cursor sort-order)
                      ;; last-cursor より後 古い順 n 件
                      :else
                      (reverse (domain.community/list-part-community (:community-query-repository repo) request-size last-cursor sort-order)))
